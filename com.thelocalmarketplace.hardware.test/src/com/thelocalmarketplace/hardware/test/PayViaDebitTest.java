...
public PayViaDebitTest(AbstractSelfCheckoutStation station) {
	...
	public void setUp() {
		  ...
		  logic.cardIssuer.addCardData(otherDebitCard.number, otherDebitCard.cardholder,
				calendar,otherDebitCard.cvv , 1000);
		  temp.addCardData(otherDebitCard.number, otherDebitCard.cardholder,
				calendar,otherDebitCard.cvv , 1000);
	...
	public void testTap() throws IOException {
		...
		for (int i =0; i<100; i++) {
			try {
		logic.station.getCardReader().tap(otherDebitCard);
			}
	...
	public void testInsert() throws IOException {
		...
		for (int i =0; i<100; i++) {
			try {
		logic.station.getCardReader().insert(otherDebitCard, "911");
	...
	public void scanAndPaywithSwipe() throws IOException {
		...
		logic.creditController.onPayWithCard();
		otherDebitCard.swipe();// swipe card

		int i = 0;
		for (i =0; i<100; i++) {
			try {
		logic.station.getCardReader().swipe(otherDebitCard); // try swipe
	...
	public void scanAndPaywithInsert() throws IOException {
		...
		logic.creditController.onPayWithCard();
		otherDebitCard.insert("911");// insert
		int i= 0;
		for ( i =0; i<100; i++) {
			try {
		logic.station.getCardReader().insert(otherDebitCard, "911");
